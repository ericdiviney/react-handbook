...
<img
  ...
  src="/framework-tweet.png"
  alt="framework recommendation on twitter"
/>
...
You can also use a framework like [Expo](https://www.npmjs.com/package/expo) to build an application that will run natively on Android, iOS, AND modern web browsers using the same codebase. We plan on building this section out a little more in the future, providing learning resources and common tips for React Native projects. Have ideas? Help us out and [pitch them](https://docs.google.com/document/d/10n5LQH_8VZFMzngMWhY74WrvzqFKF3v-9iUPpiMxMDQ/edit#) in our git repository.

### **Best Practices**

### Responsiveness
When it comes to responsiveness, you can use `react-native-normalize` to calculate your responsive units. Example:
...
### Stateless is best
Stateless React components have a much small memory footprint than stateful ones. Tracking memory usage in React Native is crucial, so structuring around this approach where `useState` is defined as low as possible within the component tree is best for local-only changes.

### Package Considerations
It is advised that you default to using packages created by the React Native Community. For a list of community-developed packages consider using the [React Native Directory](https://reactnative.directory/).

### Imports

Creating aliases for cleaner imports is amazing. Consider using `babel-plugin-module-resolver`. To avoid long-nested imports (e.g., `import Component from '../../long/nested/import/direcotry/components/Component'`) which can make this into a much cleaner import such as `import { MyComponent } from '@components'`.

### Platform-specific Handling Code
Occasioanlly you will run into scenarios where Platform-specific styles or operations need to occur. React Native provides a `Platform` object to help with this.

**Platform Handling** - Consider the following scenario where we can use `Platform` to handle font styles which are native to their platform.
...
You may also access the `Platform.OS` property to see if the platoform is `ios`, `android`, `macos`, `windows`, or `web`.

**Native Modules** - Furthermore, there will be times you need to use `Native Modules` to interact with device-specific APIs that JavaScript cannot provide. At the time of writing, `Native Modules` and `Native Components` are deprecated and will eventually transfer to the new and improved `Turbo Native Modules` and `Fabric Native Components`. For more info, visit the [Native Modules Intro](https://reactnative.dev/docs/native-modules-intro) from the docs.

### File Structure
 Creating a divided component folder structure into two different directories: `/containers/` and `/components/`.

 **Containers** - They generally are used as Higher-Order Components (HOCs) to access to global state entities such as Redux actions or hooks. They do not contain react-native imports that you would use in JSX (e.g., View, Text). They also may contain anything `react-navigation` or a library unique to the project outside of the usual react-native ecosystem.
...
import { Formik } from 'formik'
 ```
 *<sup>Container import code</sup>*

 **Components** - This is more in line with a traditional React structure. These are children to Containers and generally speaking receive data from those Cotainers. This is where most of your JSX code is intended to live with react-native specific code. Any other code for reuse like styles and components make sense here. This is also the best opportunity space for using hooks.

 ```jsx
import React, { useRef, useState } from 'react'
import { View, Text } from 'react-native'
import MyCustomComponent from './MyCustomComponent'
import type { CustomComponentType } from 'types/custom/components'
import { customComponentStyles as styles } from './styles'
```
*<sup>Component import code</sup>*

### Rely on `SafeViewArea`

Devices have a tendency to change how the screen is layed out, be it iOS or Android. To ensure consistency across device generations, and to limit the amount of visual handling, always include the `SafeViewArea` to parent your views.

### Action Handling
 **Cancellation** - Give your user an exit strategy while in the middle of performing an unintended action. This can be done by handling a mid-touch gesture as the user is dragging their finger away from the action space.
 
**Stick to Buttons With Visual Feedback** - Various buttons provided by React Native have their own feedback controls which provide visual aid to the user. It is [recommended best practice](https://reactnative.dev/docs/gesture-responder-system) to implement visual feedback for a gesture touch to keep the user informed of action results. This generally can be performed with a simple animation. There are better alternatives like `TouchableHighlight`, `TouchableOpacity`, and `TouchableNativeFeedback`. `TouchableWithoutFeedback` is to be avoided if you truly do not want to trigger an animation.

### **Performance & Optimization**
With React Native being a leader in producing mobile applications, it is imperative to consider performance caveats and best practices. For more, please refer to the [React Native Performance Overview Docs](https://reactnative.dev/docs/performance). Below are some neat tricks for optimization.
...
- **Proper image Formats** - You may want to consider using formats like PNG or WEBP format for reducing binary size of images for both platforms. This can reduce images to $\frac{1}{3}$ of the original size.

- **`FlatList`** **is your friend**. You may be tempted to use a `ScrollView` for loading a large list of items. Do not. `FlatList` uses lazy-loading for better memory performance.
...
### **State**
**Global State Libraries** - Using a global state management library like `redux-toolkit` is highly recommend to optimize for performance and limit re-renders.

### **Security**
You can create very scary scenarios with an insecure app, and in many sectors like Health, Financial, and E-commerce it is critical that protections are in place.

**Storing Sensitive Information** - While Async Storage is great in most situations, for secure storage of data that may be Personally Identifying Information (PII) or access tokens, it is best to use a package like [Encrypted Storage Package](https://github.com/emeraldsanto/react-native-encrypted-storage) which interacts with `EncryptedSharedPreferences` (Android) and `Keychain` (iOS) for storing data in an encryted format to keep attackers out and your users safe. 
