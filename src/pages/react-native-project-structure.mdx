...
**Cancellation** - Give your user an exit strategy while in the middle of performing an unintended action. This can be done by handling a mid-touch gesture as the user drags their finger away from the action space.

**Stick to Buttons With Visual Feedback** - Various buttons provided by React Native have their own feedback controls which provides visual aid to the user. It is [recommended best practice](https://reactnative.dev/docs/gesture-responder-system) to implement visual feedback for a gesture touch to keep the user informed of action results. This generally can be performed with simple animations. There are better alternatives like `TouchableHighlight`, `TouchableOpacity`, and `TouchableNativeFeedback`. `TouchableWithoutFeedback` is to be avoided if you truly do not want to trigger an animation.
...
**Holistic Security Approaches** - Before setting off you should probably read the [Security](https://reactnative.dev/docs/security) guidelines provided by the React Native docs.
