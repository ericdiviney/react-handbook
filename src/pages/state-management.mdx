...
export const sections = [
    ...
    { title: '3) Global State Libraries', id: 'table-comparison', indent: true },
...
While many React educators on the web and even the React docs recommend [Context](https://react.dev/learn/passing-data-deeply-with-context) for global-like state management - we find that unless your application is very simple and will remain very simple - context is a very limiting state management solution.

Read more in our upcoming advanced State Management Guide why we think there are better options, but also when Context IS the best tool for the job.
...
Benefits
- Granularity: splitting global state into smaller chunks allows for faster updates and easier memoization for accessing as well
- Efficient: since updates can be made to each "atom" independently, in the context of a React app this leads to a very optimized number of re-renders
...
Tradeoffs
- Proper Coordination: properly cascading global state changes across atoms can involve more complexity than a singleton approach
...
- Limited middleware and devtools - is this true? Ask Josh about both of these?
...
FLUX: Immutability & Unidirectional Data-Flow {{ className: 'lead' }}

***DESCRIPTION OF FLUX, HOW ITS USED IN A LOT OF LIBRARIES HERE, BUT THESE ARE ALL INDEPENDENT ARCHITECTURES ON THEIR OWN***
