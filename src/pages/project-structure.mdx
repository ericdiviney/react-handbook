...
It is a goldmine of great ideas to help you keep your codebase simple and clean (while also scaling well for very large applications). I tend to refer to this repo after I've determined I need a particular feature/technology when I'm asking "how/where" I should put these features/technology in my project. Not the "why" - but the "where".

## Folder Structure

If you need a scalable folder structure for your React SPA, I recommend this one from [bulletproof-react](https://github.com/alan2207/bulletproof-react/blob/master/docs/project-structure.md). If you use a meta-framework, these conventions will likely still work fine (keep in mind server-side rendering of React components might require a tweak to this). Having said that, there are some slightly unique things to be aware of when building with [Next.js](https://github.com/alan2207/bulletproof-react/issues/21) or [Remix](https://remix.run/docs/en/1.14.0/file-conventions/route-files-v2#folders-for-organization) (both frameworks are moving towards "nested routes" which is a way to avoid network request waterfalls that slow our applications down - doing this correctly might require some changes to your folder structure, but it will probably be worth it in terms of the increase in performance you'll see).
...
At a minimum, I always have an eslint configuration in my projects, but I recommend running both (eslint is more for catching code smells and potential bugs, and prettier is for opinionated formatting).

## Maintaining Documentation

Getting engineers to properly document their code can sometimes be difficult (and even slow down team productivity).
...
1. Use [TypeScript](https://www.typescriptlang.org/) - TypeScript can create all sorts of documentation that your IDE (like [Visual Studio Code](https://code.visualstudio.com/docs/languages/typescript)) can relay to you while coding. Using TypeScript properly means that all the engineers on your team can:
    1. Analyze function signatures, parameters, and their return types without leaving the file you are working in
    2. Auto-complete typing
    3. More!
...
- Writing jsdoc-like comments above non-react functions that you define in your codebase
    - I would probably only write jsdoc comments on functions I define that are NOT React components or something that returns JSX (utility functions, functions that are business/domain logic, data-fetching, etc.)
