

export function StateRequest() {
	return (
		<div className="inset-x-0 bottom-0 py-3 pointer-events-none">
			<div className="max-w-xl p-6 bg-white shadow-lg pointer-events-auto dark:shadow-inset rounded-xl dark:bg-zinc-900 ring-1 ring-gray-900/10 dark:ring-zinc-800">
				<div className="flex flex-col gap-4">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-12">
						<path stroke-linecap="round" stroke-linejoin="round" d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z" />
					</svg>
					<p className="m-0 text-sm leading-6">
						Due to lack of experience, we currently cannot cover approaches like state machines or signals. If you have experience with these architectures we would love to have you as a <a href="https://github.com/ericdiviney/react-handbook" className="font-semibold text-sky-400">contributor</a>.

						Looking for writers that can contribute learning paths for:
						<ul>
							<li>
								<a href="https://signia.tldraw.dev/" className="font-semibold text-sky-400">signia</a>
							</li>
							<li>
								<a href="https://legendapp.com/open-source/state/" className="font-semibold text-sky-400">legend</a>
							</li>
							<li>
								<a href="https://xstate.js.org/docs/" className="font-semibold text-sky-400">xState</a>
							</li>
						</ul>
					</p>
				</div>
			</div>
		</div>
	)
}